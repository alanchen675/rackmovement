...
class RMPEnv:
    ...
    def reset(self):
        ...
        if self.prev_pos_rack_map.shape[2]==1:
            ...
            self.init_pos_rack_map = self.one_hot_encode(duplicated_prev_pos_rack_map, self.num_rack_types, True)
        else:
            self.pos_rack_map = self.one_hot_encode(self.prev_pos_rack_map, self.num_rack_types, True)
    ...
    def step(self, selected):
        ...
        info_list = self.comp_heuristics(selected)
        #self.comp_heuristics_parallel(selected)
        done = (self.selected_count == self.problem_size)
        if done:
            reward, move_cost, reward_list = self.get_reward()  # note the minus sign!
            reward = -reward
            reward_list = [-r for r in reward_list]
        else:
            reward = None
            move_cost = None
            reward_list = None

        return self.step_state, reward, done, info_list, move_cost, reward_list

    def obj(self, X, quad_penalty=True):
        ...
        S2XRmin = torch.minimum(torch.zeros_like(self.level2_res_limit, device='cuda'),\
                                self.level2_res_limit.unsqueeze(0).unsqueeze(0)- S2XR)
        S1XRmin = torch.minimum(torch.zeros_like(self.level1_res_limit, device='cuda'),\
                                self.level1_res_limit.unsqueeze(0).unsqueeze(0)- S1XR)
        #self.logger.info(f'The shape of tensor after min is {SXRmin.shape}')
        if quad_penalty:
           first_term = ((-SXRmin)**2).sum(dim=(2,3))
           third_term = ((-S2XRmin)**2).sum(dim=(2,3))
           forth_term = ((-S1XRmin)**2).sum(dim=(2,3))
        else:
            first_term = F.softplus(-3*SXRmin, beta=20).sum(dim=(2, 3))
            third_term = F.softplus(-3*S2XRmin, beta=20).sum(dim=(2, 3))
            forth_term = F.softplus(-3*S1XRmin, beta=20).sum(dim=(2, 3))
        second_term = 1e2 * torch.std(SXR, dim=2).sum(dim=-1)

        first_term_vio = -torch.sum(SXRmin)
        third_term_vio = -torch.sum(S2XRmin)
        forth_term_vio = -torch.sum(S1XRmin)
        ...
        return [first_term, second_term, third_term, forth_term, first_term_vio, third_term_vio, forth_term_vio]

    def get_reward(self):
        reward_list = self.obj(self.pos_rack_map)
        reward = sum(reward_list[:4])
        self.pos_rack_map = self.one_hot_to_categorical(self.pos_rack_map)
        # Move the second dimension to the third dimension
        #current_allocations = torch.sum(self.init_pos_rack_map, dim=2)
        #allocations = current_allocations-self.demand.unsqueeze(1)
        #steps = torch.sum(torch.abs(allocations), dim=2)
        #moves = current_allocations[:,0,:]//self.num_rack_types
        if self.prev_pos_rack_map.shape[2]==1:
            ...
            reshaped_prev_map = None
        mask = mask.float()
        # The movement cost does not make sense for the experiment with only one step.
        move_cost = torch.sum(mask, dim=2)
        #move_cost -= moves*10
        #move_cost -= steps*10
        # So, the movement cost is excluded from the reward.
        reward += move_cost
        ...
        return reward, move_cost.cpu().detach().numpy(), reward_list
    ...
    def comp_heuristics(self, action):
        ...
        for batch_id in range(self.pos_rack_map.shape[0]):
            ...
            for pomo_id in range(self.pos_rack_map.shape[1]):
                rack_type = action[batch_id, pomo_id]
                info = self.comp_sequential_put(self.pos_rack_map[batch_id, pomo_id],\
                    rack_type, self.demand[batch_id, rack_type])
                pomo_infos.append(info)
            info_list.append(pomo_infos)
        return info_list

    def f(self, X, quad_penalty=False):
        ...
        sub_scope_rack_res = self.res_limit[0,0]
        if quad_penalty:
            first_term = (torch.clamp(SXR-sub_scope_rack_res, min=0)**2).sum()
            third_term = (torch.clamp(S2XR-self.level2_res_limit, min=0)**2).sum()
            forth_term = (torch.clamp(S1XR-self.level1_res_limit, min=0)**2).sum()
        else:
        ...
        obj_list = [first_term.cpu().detach().item(),\
                    second_term.cpu().detach().item(),\
                    third_term.cpu().detach().item(),\
                    forth_term.cpu().detach().item()]
        obj = first_term + second_term + third_term + forth_term
        #return obj
        return obj, obj_list
    ...
    def comp_sequential_put(self, X, rack_type, demand):
        ...
        for i in range(moves):
            ...
            if occupied_indices.any():
                delta_x = torch.zeros_like(X[:, rack_type])
                delta_x[occupied_indices] = -1
                grad_delta = X.grad[:, rack_type] * delta_x
                # Apply mask to gradients and find the index with the largest gradient in column i
                masked_grads = grad_delta.masked_fill(~valid_indices, float('-inf'))
                #masked_grads = X.grad[:, rack_type].masked_fill(~valid_indices, float('-inf'))
                selected_pos = torch.argmax(masked_grads)
                ...
                delta_x[valid_indices] = 1
                grad_delta = X.grad[:, rack_type] * delta_x
                masked_grads = grad_delta.masked_fill(~occupied_indices, float('inf'))
