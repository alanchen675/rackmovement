...
class Config:
    def __init__(self, num_rack_types=10, num_pos=1000):
        self.num_rack_types = num_rack_types
    ...
    def initialize_dimensions(self, setting):
        ...
        if setting==1:
            self.num_positions = self.num_pos
            #self.num_rack_types = 20
            self.num_groups = 4
            self.num_scopes = 50
    ...
    def initialize_racks_v2(self):
        ...
        random_numbers_7rts = np.random.rand(self.num_rack_types-3)
        random_numbers_3rts = np.random.rand(3)
        normalized_numbers_7rts = random_numbers_7rts / random_numbers_7rts.sum()
        normalized_numbers_3rts = random_numbers_3rts / random_numbers_3rts.sum()
        normalized_numbers_7rts *= 2.0/7*0.5
        normalized_numbers_3rts *= 5.0/7*0.5
        self.rack_pos_mapping_prob = normalized_numbers_7rts.tolist()
        self.rack_pos_mapping_prob.insert(5, normalized_numbers_3rts[0])
        self.rack_pos_mapping_prob.insert(7, normalized_numbers_3rts[1])
        self.rack_pos_mapping_prob.append(normalized_numbers_3rts[2])
        self.rack_pos_mapping_prob.append(0.5)
    ...
    def initialize_constraints(self, setting):
        ...
        num_res_types = len(self.resource_weights)
        # replace the self.num_rack_types in self.L.shape, self.level1_L.shape, and self.level2_L.shape with num_res_types
        if setting==1:
            self.L = np.random.randint(self.sscale_l3_resmin, self.sscale_l3_resmax, size=(self.num_scopes, num_res_types))
            self.level2_L = np.random.randint(self.sscale_l2_resmin, self.sscale_l2_resmax, size=(self.num_level2_scopes, num_res_types))
            self.level1_L = np.random.randint(self.sscale_l1_resmin, self.sscale_l1_resmax, size=(self.num_level1_scopes, num_res_types))
        else:
            self.L = np.random.randint(self.lscale_l3_resmin, self.lscale_l3_resmax, size=(self.num_scopes, num_res_types))
            self.level2_L = np.random.randint(self.lscale_l2_resmin, self.lscale_l2_resmax, size=(self.num_level2_scopes, num_res_types))
            self.level1_L = np.random.randint(self.lscale_l1_resmin, self.lscale_l1_resmax, size=(self.num_level1_scopes, num_res_types))
            #self.L = np.random.randint(2, 5, size=(self.num_scopes, num_res_types))
            #self.level2_L = np.random.randint(30, 50, size=(self.num_level2_scopes, num_res_types))
            #self.level1_L = np.random.randint(70, 90, size=(self.num_level1_scopes, num_res_types))
